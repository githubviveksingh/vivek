...
if(isset($_POST['submit'])){
    $contentArray = array();
    $id = $_POST["smid"];
    $contentArray["MSISDN"] = $_POST['MSISDN'];
    $contentArray["IMEI"] = $_POST['IMEI'];   
    $contentArray["serviceProvider"] = $_POST['serviceProvider'];
    $contentArray["APNValue"] = $_POST['APNValue'];
    $contentArray["BillPlan"] = $_POST['BillPlan'];
    ...
    $msidn          = $_POST['MSISDN'];
    $apn            = $_POST['APNValue'];
    $billpan        = $_POST['BillPlan'];
    
    $empname        = ucwords($empDetails['name']);
    $empemail       = $empDetails['email'];
    /* Get SIM information */
    $simInfo        =  getSimInfo($id);

        if($simInfo[0]['empID'] != $contentArray["empID"]){

            $contentArray['alote_status'] = 0;

            $currentEmpDetails     = getEmpDetails($simInfo[0]['empID']);
            $currentEmpemail       = $currentEmpDetails['email'];

            $auditData = array("old_values"=>$_SESSION['oldData'], "new_values"=>$contentArray);
            $auditID   = generateAudit("SIM_ANOTH", $smid, $auditData, "");

            $mailbox        = file_get_contents("partial-forms/alotted-sim-to-other-employee.html");
            ...
            $subject        = "Process Portal- allotted sim assign to another employee";
            $mailSender     = mailSend($currentEmpemail,$subject,$mailbox);

        }
	}
	if(!empty($_POST['hrdDevice']))
	{
	$contentArray["hrdDevice"] = $_POST['hrdDevice'];
	}

    if($smid != "0"){
	$updateSim = updateData($table, $contentArray, "identifier", $id);
    $auditData = array("old_values"=>$_SESSION['oldData'], "new_values"=>$contentArray);
    $auditID   = generateAudit("SIM_UPD", $smid, $auditData, "");
    if(!empty($_POST['EMPID'])){
        if($updateSim){
            if($simInfo[0]['empID'] != $contentArray["empID"]){
            /* Send Mail */
                $mailbox        = file_get_contents("partial-forms/alotted-sim-employee.html");
                ...
                $TITLE          = 'allotted a SIM';
                $mailbox        = str_replace("[TITLE]" , $TITLE , $mailbox);
                
                
                $subject        = "Process Portal- allotted sim";
                $mailSender     = mailSend($empemail,$subject,$mailbox);

                $_SESSION["success"] = "Sim allotted Successfully.";
                // header("Location: sim-inventory.php");
                // exit();
            }
        }else{
            $_SESSION["success"] = "Sim could not be allotted. Please try again later.";
            header("Location: update-sim-inventory.php");
            exit();
        }
    }
...
if($smid != "0"){
    ...
    if(count($empData)){
        $MSISDN             = $empData["MSISDN"];
        $IMEI               = $empData["IMEI"];
        $serviceProvider    = $empData["serviceProvider"];
        $APNValue           = $empData["APNValue"];
        $BillPlan           = $empData["BillPlan"];
        $statusCode         = $empData["statusCode"];
		$hrdDevice          = $empData["hrdDevice"];
		$CustomerID         = $empData["customerID"];
		$EMPID              = $empData["empID"];	
        $heading            = "Update Sim Inventory Item Details";
    }else{
       $_SESSION['error'] = "Item does not exist.";
       header("Location: sim-inventory.php");
        exit();
...
?>
    <body class="page-header-fixed page-sidebar-closed-hide-logo page-content-white">
                                         ...
                                         <?php
                                            ...
                                            <div class="form-body">
                                                <div class="form-group">
                                                    <label class="col-md-3 control-label">IMSI</label>
                                                    <div class="col-md-4">
                                                        <input class="form-control" placeholder="Enter IMSI" type="text" id="IMSI" name="IMEI" value="<?php echo $IMEI;?>">
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                    <label class="col-md-3 control-label">MSISDN</label>
                                                    <div class="col-md-4">
                                                        <input class="form-control" placeholder="Enter  MSISDN" type="text" id="MSISDN" name="MSISDN" value="<?php echo $MSISDN;?>">
                                                    </div>
                                                </div>
												<?php include"partial-forms/service-provider-list.php";?>
                                                <div class="form-group">
                                                    <label class="col-md-3 control-label">APN Value</label>
                                                    <div class="col-md-4">
                                                        <input class="form-control" placeholder="Enter APNValue" type="text" name="APNValue" id="gstNo" value="<?php echo $APNValue;?>">
                                                    </div>
                                                </div>
                                                 <div class="form-group">
                                                    <label class="col-md-3 control-label">Bill Plan</label>
                                                    <div class="col-md-4">
                                                        <input class="form-control" placeholder="Enter BillPlan" type="text" name="BillPlan" value="<?php echo $BillPlan;?>">
                                                    </div>
                                                </div>     
                                            ...
                                            <div class="form-actions fluid">
                                                <div class="row">
                                                    <div class="col-md-offset-3 col-md-9">
														...
														<a class="btn yellow" href="/sim-inventory.php">Back</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </form>
        ...
        <script src="assets/pages/update-sim-inventory.js"></script>
        <script>
		...
		function showinventoryfileds(str) {
			...
			if (this.readyState == 4 && this.status == 200) {
			 ...
			 xmlhttp.open("GET","includes/getExtraFiledsInventory.php?q="+str+"&q2=SIM",true);
			 xmlhttp.send();
			}
			$("hrdDevice").select2();
		}
