...
$query .= " FROM tblHardware WHERE 1 = 1 AND EMPID='$emp'";
$counterQuery = $query;
...
foreach($data as $d){
    ...
    $countVal++;
    $array[] = $d['IMEI'];
    $array[] = $PRODUCTCAT[$d["productCat"]][1];
    $audit=audit_details_Inventory($d["identifier"],'HRD_UPD','EMPID');
	$array[] = getEmpDetails($audit["triggeredBy"])['name'];
	$array[] = $audit["createdOn"];  
    if($d['alote_status']==0){
        $array[] = "<span id='alotAction'><a href='accept-hrd-inventory.php?mcid=".$d['identifier']."&empID=".$d['EMPID']."' id='acceptAlot' class='btn green'>Accept</a> | <a href='reject-hrd-inventory.php?mcid=".$d['identifier']."&empID=".$d['EMPID']."' id='acceptAlot' class='btn red'>Reject</a></span>";
