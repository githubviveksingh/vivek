...
$emp=$_GET['emp'];
$customColumns = array("classification"=>"MACHINEANDTOOLCLASS");
...
$query .= " FROM tblMachine WHERE 1 = 1 AND employeeID='$emp'";
$counterQuery = $query;
...
foreach($data as $d){
    ...
    $countVal++;
    $array[] = $d['name'];
    $array[] = $MACHINEANDTOOLCLASS[$d["classification"]][1];
    $audit=audit_details_Inventory($d["identifier"],'MCN_UPD','employeeID');
	$array[] = getEmpDetails($audit["triggeredBy"])['name'];
	$array[] = $audit["createdOn"];  
    if($d['alote_status']==0){
        $array[] = "<span id='alotAction'><a href='accept-machine.php?mcid=".$d['identifier']."&empID=".$d['employeeID']."' id='acceptAlot' class='btn green'>Accept</a> | <a href='reject-machine.php?mcid=".$d['identifier']."&empID=".$d['employeeID']."' id='acceptAlot' class='btn red'>Reject</a></span>";
