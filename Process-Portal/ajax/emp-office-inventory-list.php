...
$query .= " FROM tblofficeitem WHERE 1 = 1 AND EMPID='$emp'";
//echo $query; die;
$counterQuery = $query;
...
foreach($data as $d){
    ...
    $array[] = $d['itemName'];
	$array[] = $d['description'];
    $audit   = audit_details_Inventory($d["identifier"],'OFC_UPD','EMPID');	
	$array[] = getEmpDetails($audit["triggeredBy"])['name'];
	$array[] = $audit["createdOn"];  
    if($d['alote_status']==0){
        $array[] = "<span id='alotAction'><a href='accept-office-inventory.php?mcid=".$d['identifier']."&empID=".$d['EMPID']."' id='acceptAlot' class='btn green'>Accept</a> | <a href='reject-office-inventory.php?mcid=".$d['identifier']."&empID=".$d['EMPID']."' id='acceptAlot' class='btn red'>Reject</a></span>";
