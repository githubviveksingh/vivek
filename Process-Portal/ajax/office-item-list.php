...
$customColumns = array("I.statusCode"=>"STATUSCODE");
...
$query .= " FROM tblOfficeItem I JOIN tblLocation L on L.identifier=I.locationID WHERE 1 = 1";
$counterQuery = $query;
...
foreach($data as $d){
    $array = array();
    $editLink = "update-office-item.php?smid=".$d["identifier"];
	...
	if($moveData!=0){
	   ...
	   $actionOption='<li ><a href="'.$editLink.'">Edit</a></li> 
				   <li><a href="javascript:void(0);" class="changelc" current-location="'.$d['locationID'].'"  itemid="'.$d['itemID'].'" ctext="'.$d['itemName'].':'.$d['description'].'">Change Location</a></li>';
	}
