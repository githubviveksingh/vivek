...
if(isset($_POST['deliveryLocation']) && $_POST['deliveryLocation']>0){  
    ...
    generateAudit("HRD_LUP", $itemID, $contentArrayAudit, $notifyAction="");

$item=getInventoryShortDetails($itemID,'201');
$empD=getEmpDetails($_SESSION["user"]["identifier"]);
$html=file_get_contents("../partial-forms/new-tech.html");
$html =str_replace("[TITLE]" , "HARDWARE Inventory" , $html);
$protocol = ((!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] != 'off') || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://";
$url = $protocol . $_SERVER['HTTP_HOST'];
$link=$url.'/hardware-inventory.php?sSearch='.$item;
